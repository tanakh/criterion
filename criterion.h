...
typedef std::tr1::function<double(sample)> estimator;
...
#define bgroup(group_name)                                              \
  if (criterion::group BENCH_MUST_BE_IN_BGROUP_ = criterion::group(group_name)) {} \
  else

#define bench(bench_name)                                               \
  for (criterion::statistics BENCH_STAT_ = criterion::statistics(bench_name, BENCH_MUST_BE_IN_BGROUP_); \
       BENCH_STAT_.lap(); )
